# Measures a Holy Quintet character scene's drawn bounding box, in its own local
# space. Same technique as tools/animania/harness/measure_character.gd: an Adobe
# Animate atlas has no authored frame size, so the origin (horizontal centre,
# vertical BOTTOM - Funkin's characterOrigin) is measured by rendering the scene
# and counting opaque pixels. Folding the result into ORIGINS in
# build_character_scenes.gd puts the character's feet on its stage marker.
#
#   xvfb-run -a --server-args="-screen 0 1920x1080x24" godot \
#       --rendering-driver opengl3 --path . res://tools/holyquintet/measure_character.tscn
extends Node2D

const SCENES := {
	"sayaka-base": "res://holyquintet_mod/characters/chr_sayaka_base.tscn",
	"gf-base": "res://holyquintet_mod/characters/chr_gf_base.tscn",
	"kyubey-big-bald": "res://holyquintet_mod/characters/chr_kyubey_big_bald.tscn",
}
...
func _measure() -> void:
	...
	print("OUT %-16s drawn=%.0fx%.0f  local_topleft=%s  origin=(%.1f, %.1f)" % [
