...
function postCreate()
{
	...
	if (Options.gameplayShaders)
	{
		...
		FlxTween.num(-0.003, 0.003, 3, {ease: FlxEase.sineInOut, type: FlxTweenType.PINGPONG}, function(num)
		{
			if (aberration == null)
				return;
			aberration.redOff = [0, -num];
			aberration.blueOff = [0, num];
		});
	}
	cutscene.add(spirit);
...
function close(event)
{
	...
	new FlxTimer().start(0.4, function(swagTimer:FlxTimer)
	{
		...
		if (cutscene.dialogueCamera.alpha > 0)
			swagTimer.reset();
		else
		{
			finished = true;
...
{
	if (!active || event.char.positionName != "left")
		return;
