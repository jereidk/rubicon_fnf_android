...
function onStageNodeParsed(event)
{
	...
	if (sprite is FunkinSprite)
	{
		...
		if (atts[0] == false)
			return; // not using !atts[0] since the game would have convert a dynamic into a null which is slower; i wonder if its the same in hscript..?  - Nex
		initDSShader(atts[1], atts[2], atts[3], atts[4], atts[5], atts[6], atts[7], atts[8], atts[9], atts[10], atts[11], atts[12], atts[13], atts[14],
			atts[15], atts[16], atts[17], sprite);
	}
	else if (sprite is StageCharPos)
	{
		var name = event.name;
		if (event.stage.characterPoses.exists(name))
			dsShaderCharsAtts[getCharPosIndex(name)] = getDSShaderAttFromNode(node);
	}
}
		...
		for (i => atts in dsShaderCharsAtts)
			if (atts != null)
				for (char in strumLines.members[i]?.characters)
				{
					if (atts[0] == false)
						continue;
					initDSShader(atts[1], atts[2], atts[3], atts[4], atts[5], atts[6], atts[7], atts[8], atts[9], atts[10], atts[11], atts[12], atts[13],
						atts[14], atts[15], atts[16], atts[17], char);
				}

public function getCharPosIndex(charPos:String):Int
	return switch (charPos)
	{
		case "dad": 0;
		case "boyfriend": 1;
		default: 2;
...
public function initDSShader(brightness:Float, hue:Float, contrast:Float, saturation:Float, color:String, angle:Float, antialiasAmt:Float, strength:Float,
		distance:Float, curZoom:Float, threshold:Float, pixelPerfect:Bool, flipX:Bool, flipY:Bool, altMask:String, maskThreshold:Float, applyAltMask:Bool,
		sprite:FlxSprite):CustomShader
{
	...
	if (altMask != null)
		dropShadow.loadAltMask(Paths.image(altMask));
