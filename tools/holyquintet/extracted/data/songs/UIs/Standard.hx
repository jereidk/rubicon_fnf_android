...
function postUpdate(elapsed:Float)
{
	...
	if (useTargetScoreMech)
	{
		...
		{
			score_reqScoreWarningTxt.text = '▼ ${targetScore - songScore}';
			score_reqScoreWarningTxt.color = 0xFF0000FF;
		}
		else if (songScore > targetScore)
		{
			score_reqScoreWarningTxt.text = '▲ ${(targetScore - songScore) * -1}';
			score_reqScoreWarningTxt.color = 0xFFFF8000;
		}
		else
		{
			score_reqScoreWarningTxt.text = '- ${(targetScore - songScore)}';
			score_reqScoreWarningTxt.color = FlxColor.WHITE;
		}
	}
	else
	{
		score_reqScoreTxt.visible = false;
		score_reqScoreWarningTxt.visible = false;
	}
...
function updateNewIconPositions()
{
	...
	var center:Float = hpbar_Meter.x + hpbar_Meter.width * FlxMath.remapToRange(healthBarPercent, 0, 100, 1, 0);
...
{
	if (iconTweens[0] != null)
		iconTweens[0].cancel();
	iconP1.scale.set(1.1, 1.1);
	iconTweens[0] = FlxTween.tween(iconP1, {'scale.x': 1.0, 'scale.y': 1.0}, ((Conductor.stepCrochet * 2) / 1000), {ease: FlxEase.cubeOut});

	if (iconTweens[1] != null)
		iconTweens[1].cancel();
	iconP2.scale.set(1.1, 1.1);
	iconTweens[1] = FlxTween.tween(iconP2, {'scale.x': 1.0, 'scale.y': 1.0}, ((Conductor.stepCrochet * 2) / 1000), {ease: FlxEase.cubeOut});
}

public function updateScoreTxtPos()
{
	if (atksSustained >= 1 && !checkStatExistance('Atks. Sustained'))
	{
		var newStatus:StatTextUI = new StatTextUI('Atks. Sustained', hpbar_Border.y);
		insert(members.indexOf(judgementDisplay) - 1, newStatus);
		statTxtGrp.push(newStatus);
		newStatus.group.cameras = [camUI];
	}

	var statTxtsTotalLength:Float = 225 * statTxtGrp.length;
	for (i in 0...statTxtGrp.length)
	{
		statTxtGrp[i].group.x = FlxG.width / 2;
		statTxtGrp[i].group.x -= (statTxtsTotalLength / 2);
		if (i > 0)
		{
			statTxtGrp[i].group.x = statTxtGrp[i - 1].group.x + 225;
		}
	}
}

function checkStatExistance(statToBeMade:String):Bool
{
	var statExists:Bool = false;

	for (stat in statTxtGrp)
	{
		if (stat.headerTxt.text == statToBeMade)
			statExists = true;
	}
...
{
	soulGemShakeTween = FlxTween.num(FlxMath.bound(10 - (purity * 10), 0, 10), 0.0, 0.25 + ((2 - purity) * 1.25), {ease: FlxEase.expoOut}, function(num:Float)
	{
		var randomizedXpos:Float = FlxG.random.float(-num, num);
		var randomizedYPos:Float = FlxG.random.float(-num, num);
		soulgemSprite.group.x = soulGemOriginalPos[0] + randomizedXpos;
		soulgemSprite.group.y = soulGemOriginalPos[1] + randomizedYPos;
	});
	soulgemSprite.soulValue = purity;
	soulgem_sanityOverlay.alpha = 1.0 - purity;
}

public function uiVisibility(group:String, steps:Float, targetAlpha:Float)
{
	var steps:Float = Conductor.stepCrochet * steps / 1000;
	if (steps == 0)
		steps = 0.0001;
	...
	{
		case 'health':
			targetGroup = healthBarGrp;
		case 'score':
			targetGroup = scoreGrp;
		case 'soulgem':
			targetGroup = soulGemGrp;
	}

	if (group != 'strums')
	{
		for (spr in targetGroup)
			FlxTween.tween(spr, {alpha: targetAlpha}, steps, {ease: FlxEase.sineOut});

		FlxTween.tween(strumUnderlay, {alpha: FlxMath.bound(targetAlpha, 0, Options.strumUnderlayAlpha / 100)}, steps, {ease: FlxEase.sineOut});
	}
	else
	{
		if (playerStrums != null)
			for (i in playerStrums)
				FlxTween.tween(i, {alpha: targetAlpha}, steps, {ease: FlxEase.sineOut});
		if (cpuStrums != null)
			for (i in cpuStrums)
				FlxTween.tween(i, {alpha: targetAlpha}, steps, {ease: FlxEase.sineOut});
	}
	...
	if (group == 'health')
	{
		for (spr in statTxtGrp)
			FlxTween.tween(spr.group, {alpha: targetAlpha}, steps, {ease: FlxEase.sineOut});
	}
}

var curMultiAnimation:String = '';

public function multiAnimation(type:String)
{
	if (curMultiAnimation == type)
		return;

	if (comboMultiTween != null)
		comboMultiTween.cancel();

	switch (type)
	{
		case 'show':
			var clipDist:Float = Std.int(noteStreakMutliAdd_mainSprite.height);
			comboMultiTween = FlxTween.num(noteStreakMutliAdd_mainSprite.clipRect.height, Std.int(noteStreakMutliAdd_mainSprite.height), 0.75,
				{ease: FlxEase.cubeInOut}, function(num:Float)
			{
				noteStreakMutliAdd_mainSprite.clipRect.height = num;
				noteStreakMutliAdd_mainSprite.y = (hpbar_Border.y - 70 + clipDist) - (num);
			});
		case 'hide':
			var clipDist:Float = noteStreakMutliAdd_mainSprite.height;
			comboMultiTween = FlxTween.num(noteStreakMutliAdd_mainSprite.clipRect.height, 0, 0.75, {ease: FlxEase.cubeInOut}, function(num:Float)
			{
				noteStreakMutliAdd_mainSprite.clipRect.height = num;
				noteStreakMutliAdd_mainSprite.y = (hpbar_Border.y - 70 + clipDist) - num;
			});
	...
	curMultiAnimation = type;
}

function padScore(target:String):String
{
	var scoreLength:String = 7 - Std.string(target).length;

	var returnedString:String = '000000'.substr(0, scoreLength) + target;
...
public function updateScoreText()
{
	if (useTargetScoreMech)
	{
		...
		{
			score_reqScoreWarningTxt.text = '▼ ${targetScore - songScore}';
			score_reqScoreWarningTxt.color = 0xFF0033FF;
		}
		else if (songScore > targetScore)
		{
			score_reqScoreWarningTxt.text = '▲ ${(targetScore - songScore) * -1}';
			score_reqScoreWarningTxt.color = 0xFFFF7700;
		}
		else
		{
			score_reqScoreWarningTxt.text = '- ${(targetScore - songScore)}';
