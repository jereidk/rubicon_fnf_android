import ui.gauntlet.GauntletBackgrounds;
import ui.gauntlet.GauntletStatDisplay;
import ui.gauntlet.GauntletModSelectorUI;
import ui.ButtonUI;
import BlurFilter;
...
var switchingBackgrounds:Bool = false;
var canControl:Bool = true;
public static var gm_curSel:Int = 0;
var gauntletBGs:Array<GauntletBackgrounds> = [];
public static var gauntletSelectedMods:Array<String> = [];
var gauntletSelectorGrp:Array<GauntletModSelectorUI> = [];
var scrollOriginalY:Array<Float> = [];
var scrollTweens:Array<FlxTween> = [];
...
var gauntletModList:Array<String> = [
	'PerformanceRegen',
	// 'EasyChart',
	'NoMechanics',
	'BiggerJudgementWindows',
	// 'PerformanceRegen',
	// 'ReducedScrollSpeed',
	// 'OneSongSkip',
	'IncreasedPerformanceLoss',
	'SmallerJudgementWindows',
	'ComboCountRequirement',
	'OpponentPerformanceDrain',
	// 'RandomNoteColors',
	// 'SixKeyCharts',
	'HarderMechanics',
	'InstantKillMechanics',
	'HigherScoreRequirement',
	'StealthNotes',
	'ZoomingNotes',
	// 'OneLife'
];
...
FlxG.sound.load(Paths.sound('ui/gauntlet/stage_increase'));
FlxG.sound.load(Paths.sound('ui/gauntlet/stage_increasetofinal'));
FlxG.sound.load(Paths.sound('ui/gauntlet/stage_decrease'));
...
FlxG.sound.load(Paths.sound('ui/gauntlet/start_peaceful'));
FlxG.sound.load(Paths.sound('ui/gauntlet/start_stressed'));
FlxG.sound.load(Paths.sound('ui/gauntlet/start_hopeless'));
function create()
{
	DiscordUtil.changePresenceSince("In Gauntlet", null);
	...
	// Setup gauntlet backgrounds
	var bgnames:Array<String> = ['peaceful', 'stressed', 'hopeless'];
	for (i in 0...bgnames.length)
	{
		var gauntletBG = new GauntletBackgrounds(bgnames[i]);
		gauntletBG.ID = i;
		add(gauntletBG);
		gauntletBGs.push(gauntletBG);
		gauntletBG.group.alpha = 0.0;
	}

	bg_frontFog = new FlxBackdrop(Paths.image('ui/gauntlet/bgs/fog'), FlxAxes.X, 0, 0);
	add(bg_frontFog);
	bg_frontFog.color = FlxColor.WHITE;
	bg_frontFog.y = 700;
	bg_frontFog.velocity.set(50, 0);

	// Setup UI
	backing_Spr = new FunkinSprite().loadGraphic(Paths.image('ui/common/back_small'));
	add(backing_Spr);
	backing_Spr.blend = BlendMode.MULTIPLY;

	multiDisplay = new GauntletStatDisplay('ScoreMultiplier');
	add(multiDisplay);

	for (i in 0...gauntletModList.length)
	{
		var gauntletMod = new GauntletModSelectorUI(gauntletModList[i]);
		gauntletMod.ID = i + 1;
		add(gauntletMod);
			...
			FlxTween.num(FlxG.camera.zoom, 5.0, 1.5, {
				...
				onComplete: function(twn:FlxTween)
				{
					var songsList:Array<Dynamic> = [
						...
						{name: "meguca", hide: false},
						{name: "reconnect", hide: false}
					];

					if (curGauntletGamemode == 'Standard')
						FlxG.random.shuffle(songsList);

					new FlxTimer().start(0.5, function(tmr:FlxTimer)
					{
						var weekData = {
							songs: songsList,
							name: "GauntletStandard",
							id: "gauntletstandard",
							difficulties: ["easy", "hard", "6k"]
						};

						PlayState.loadWeek(weekData, diff);
						PlayState.isGauntletMode = true;
...
function getMod(target:String):GauntletModSelectorUI
{
	var targetGauntletMod:GauntletModSelectorUI = null;
	for (gauntletMod in gauntletSelectorGrp)
	{
		if (gauntletMod.data.nameKey == target)
		{
			targetGauntletMod = gauntletMod;
		}
	}
	return targetGauntletMod;
}

function pairStatus(firstMod:String, secondMod:String, bonusAmt:Float):Bool
{
	if (getMod(firstMod).enabled && !getMod(secondMod).enabled)
	{
		getMod(secondMod).bonusAvaliable = true;
	}
	else if (getMod(secondMod).enabled && !getMod(firstMod).enabled)
	{
		getMod(firstMod).bonusAvaliable = true;
	}
	else if (getMod(secondMod).enabled && getMod(firstMod).enabled)
	{
		getMod(firstMod).bonusAvaliable = false;
		getMod(secondMod).bonusAvaliable = false;
		curGauntletMultiplier += bonusAmt;
	}
	else
	{
		getMod(firstMod).bonusAvaliable = false;
...
function showLeaderboards()
{
	...
	for (i in 0...lbData.scores.length)
	{
		...
		// add(overlay).cameras = [lbuiCam];
		lbgroup.add(overlay).cameras = [lbuiCam];
		overlay.screenCenter(FlxAxes.X);
		overlay.alpha = 0.75;

		nameTxt = new FlxText(0, 0, 960 - 20, lbData.scores[i].user, false);
		...
		durationAgoTxt = new FlxText(0, 0, 960 - 20, lbData.scores[i].stored, false);
		...
		scoreTxt = new FlxText(0, 0, 960 - 20, FlxStringUtil.formatMoney(lbData.scores[i].score, false));
