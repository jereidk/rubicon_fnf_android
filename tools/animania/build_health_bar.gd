...
func _init() -> void:
	...
	# The icons ride the HEALTH-DRIVEN follow point, which is what `healthBar.centerPoint`
	# is: FlxBar's centre point is where the fill boundary currently sits, not the bar's
	# geometric middle. phone-call.script pins both icons to it every frame -
	# `icon.x = centerPoint.x - 26` on one side and `centerPoint.x - (width - 26)` on the
	# other - so they travel with the seam and straddle it.
	#
	# A round of this port read centerPoint as a fixed middle and parked them on a second
	# follow point at progress_ratio 0.5. That was an over-correction to a report that they
	# sat too far APART: the separation was the ICON_POSITION_OFFSET algebra and is fixed
	# where it belongs, on the sprite offset. Parked, they ended up bunched at one END of
	# the arc - 0.5 along this curve is not its middle - and stopped answering the health
	# at all, which is what a device run then reported as the bar pointing the wrong way.
	#
