...
## 8r. doIntroAnim, and a third hxlines bug that reads UTF-16 as ASCII

The user's words were "hay una animación completa en el intro del estado" — there is a
whole animation in the state's intro. There is, and the port had none of it. What the port
did at the 0.5 s mark was set two sprites visible. What the mod does is play a six-frame
animation of the TV switching on and hang three callbacks off it.

### The bug that hid it

`doIntroAnim` (0x34bbf20) is two `FlxTimer`s, 0.5 s and 1 s. The 0.5 s closure
(0x34b8df0) is lines 1601-1607:

    1601  diskPlayer.visible = true
    1602  diskPlayer.animation.play('y')
    1603  tvSprite.visible = true
    1604  tvSprite.animation.play('y')
    1605  tvSprite.animation.onFrameChange.add(_ -> tvBackBG.visible = true)
    1606  tvSprite.animation.onFinish.addOnce(_ -> introDone())
    1607  diskPlayer.animation.onFinish.addOnce(_ -> diskPlayerMask.visible = true)

Line 1604 plays `'y'` on the TV. But hxlines said `buildBg` line 1279 registered the TV's
animation as `addByPrefix('f', ...)`, and `'f'` is not `'y'`, so the read looked like a
mod bug: a play of an animation that does not exist. That reading was wrong, and the
tool was the one lying.

hxcpp stores a string literal as UTF-8 when it is pure ASCII and as **UTF-16LE** when it
is not. `hxlines.Reader.at` read every pointer as a NUL-terminated byte string, so a
UTF-16 literal — `f\0r\0e\0e\0p\0l\0a\0y\0 \0t\0v\0 \0>\x041\x04@\x04...` — stopped at
the second byte and came back as `'f'`. Not an error, not a warning: a one-letter string
that looks exactly like the short animation names this codebase actually uses (`'a'`,
`'y'`, `'f'`). The real call is

    addByPrefix('y', 'freeplay tv образец 1', 24, false)

with `'y'` at 0x5c28aa8 (the name, length 1) and the 21-character UTF-16 string at
0x5c29878 (the prefix, matching all six `freeplay tv образец 1000X` frames).

`Reader.at` now tries UTF-16 first, on the signature "two ASCII characters in a row each
with a zero high byte". The first attempt used only the FIRST character, and that
regressed `'y'` into `'yݽ瞋'` — a one-letter ASCII string followed by unrelated bytes
satisfies a one-character test. Two characters is enough to separate them, and both
call sites now read right. This is the third hxlines bug of this shape (see 8d): it is
always the same failure mode, a wrong answer that looks like a reasonable one.

### `looped` is explicitly false

The fourth argument of that `addByPrefix` matters. On the stack at the call site,
`frameRate` is `Null<double>{flag 0, 24.0}` — flag 0 meaning PRESENT — one `Null<bool>`
is `{0, 0}`, and two are `{1, ...}`, the nulls. So `Looped = false`, `FlipX`/`FlipY`
default.

It has to be false, and the callbacks prove it independently: in flixel `onFinish` only
fires when a NON-looping animation completes, and the mod hangs two callbacks off
`onFinish`. A looping TV would also never stop flickering. `freeplay_tv_frames.tres` and
`freeplay_player_frames.tres` shipped with `loop: 1` and are now `loop: 0`. Neither is
generated by `build_freeplay_scene.gd` — they are versioned resources — so anyone
regenerating them has to put that back.

### introDone

`introDone` (0x34bc2a0) hangs off the TV's `onFinish`, so it runs at roughly 0.75 s:

    1666  tvIntroDone = true;  allowInput = true;  freeplayScore.updateScore(0)
    1680  callOnScripts('onIntroDone')

Both booleans are `movb $0x1` on 0x1c8 and 0x2d8.

### allowInput started on the wrong value

`allowInput` (0x2d8) is written in five places. Four were already understood; the fifth
is `FreeplayScreen::__construct` at 0x34d327b, `movb $0x0` — it starts **false**. The
port had `var allow_input: bool = true`, which let keys through during the whole switch-on
and, worse, made an earlier note in the port wrong in the other direction: the comment on
line 1651 claimed `changeSelection` at 1648 bailed out on its own guard. It does not,
because `introDone` opened input a quarter of a second earlier. Both are fixed, and line
1644's second `allowInput = true` stays where the mod has it.

### Line 1612 counted instead of guessed

While in the 1 s closure: line 1612 sets exactly FIVE things visible, and the five
`mov <offset>(%rax),%rax` + `call *0x128` pairs at 0x34ca150 name them — 0x258 dotsGrp,
0x250 selectorsGroup, 0x178 tvGlow, 0x1d0 tvNoiseBack, 0x1f0 tvNoiseForward. The port's
list had `PlayerLayer` in it, which is `diskPlayerMask` and belongs to line 1607, and
`ShadowsOnBed`, which is line 1645 and goes after the sound.

`Disks` stays in that closure with a `?`: `grpDisks` (0x238) is set invisible by buildBg
1367 and there is no `set_visible` on 0x238 anywhere in the class. Something outside
FreeplayScreen shows the carousel and it has not been found.

### Three node names that were all wrong

`disk_player`, `disk_player_mask` and `tv_back_bg` were declared, bound with
`get_node_or_null` to `DiskPlayer`, `DiskPlayerMask` and nothing at all, and never read.
The scene calls them `Player`, `PlayerLayer` and `TvBackBG`. Three silent nulls that cost
nothing while the fields were dead and would have cost the whole intro the moment they
were used. Same shape as the three found in 8g — it is worth grepping every
`get_node_or_null` in this file against the scene, once, rather than finding them one
crash at a time.

`tools/animania/harness/intro_strip.gd` renders the sequence at seven timestamps and
prints each sprite's frame, so the animation can be watched rather than assumed:

    t=0.75  tv(vis=true frame=0 playing=true)   back=false mask=false allow=false
    t=0.88  tv(vis=true frame=1 playing=true)   back=true  mask=false allow=false
...
`A_B_TOP` is the one entry that is **not** in `FlxVirtualPad`, and it is marked as such in
the file: the same row the reference puts at the bottom right, hung 87 px below the top edge
instead, clearing the 76-px score bar. It invents no coordinate of its own.
...
### The week menu ends with no pad at all

It got there in two steps, and the first one was wrong in an instructive way.

Step one made the **difficulty arrows** tappable — the `◄ ►` either side of the value, which
in the mod are decoration (`changeDifficulty` is driven by the keyboard and the arrows only
announce that it can be), with the same 132 px floor on the finger box that freeplay uses,
since the drawing is 25×49 on screen. The week titles were already tappable. The pad was then
cut down to two buttons on the argument that both walks **wrap** (`wrapi` in `change_level`
line 340 and `change_difficulty` 626), so one button per axis still reaches everything.

That was still a pad on a screen that no longer needed one. Step two removed it: **the week
menu is now driven entirely by what it already draws** — tap an arrow to change difficulty,
tap a week's title to select it, tap **BF** to go in. Going back is the phone's own back
button, which `story_menu.gd` already answers (`NOTIFICATION_WM_GO_BACK_REQUEST`). The
`LEFT_DOWN` half-cross that step one added is gone with it; a mode no screen uses is a
liability, and the reasoning survives here.

Two decisions inside that are worth stating. **Tapping the already-selected week does
nothing** — everywhere else in this port a second tap on the selection is the confirm, but
here the confirm has its own place, and half this screen is huge week titles that a thumb
lands on constantly. And **BF is found by name, in pieces**: the asset paths are
`BF_STANDART_MENU` and `X-MAS_BF_MENU`, so the prop is matched on a whole `BF` token after
splitting on non-alphanumerics, not with a `contains("BF")` that will one day match something
else. `GF_STANDART_MENU` is the reason that matters.

**The action buttons moved before they left, and the harness is why.** While the pad was
still there, `story_touch_probe.gd` measured the overlap in pixels: `b` sat on 20 196 px² of
the difficulty sign, `a` on 2 065 px² of it **plus 1 262 px² of the right arrow**. That last
number is the interesting one. Tapping that arrow changed the difficulty *and* fired the
pad's `a` underneath it, which is ENTER, which starts the week — and from there
`change_difficulty` early-returns on `_confirmed`, so the next tap did nothing at all. From
outside that reads as "the left arrow doesn't work", which is exactly how it first showed up.
No screenshot would have found it.

The probe now checks the five things the screen promises — each arrow changes the difficulty
*and does not confirm*, another week's title selects, the selected one does nothing, BF
confirms — plus that the screen carries no `MenuVirtualPad`, so that re-hanging one shows up
as a failure. It also lands on a **playable** week before tapping BF: `select_level` plays
the locked sound and returns when a week has no built songs (line 367), and in this port only
tutorial is built, so the first version of the check blamed the tap for a locked week.

Opacity for the pads that remain drops from the reference's 0.6 to **0.5**: that pad is drawn
over a game whose art keeps clear of the corners, and this one's does not.
...
what breaks a badly written pad. Zero failures on options_screen, which is also its default
target: the main menu, freeplay and now the week menu carry no pad, so pointing it at any of
them reports a `FALLO` that is not one — those three have `freeplay_touch_probe.gd` and
`story_touch_probe.gd` instead. Its two-finger test takes the first two buttons of whatever
cross the screen has rather than two named ones; asking a two-button cross for an `up` was a
null the harness swallowed in a spin, which is "it hangs" wearing the costume of "it is slow"
for the second time. Two screens it cannot test and says so: credits, because accepting or going
