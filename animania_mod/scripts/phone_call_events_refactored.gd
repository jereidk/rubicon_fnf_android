...
## ─── Exports ────────────────────────────────────────────────────────────────
## camera, clock, hud, bumper, cast, the lanes, the camera points, hud_root,
## hud_up/hud_down and the overlay rects are all exported by song_events.gd.
## Only phone-call's own furniture is declared here.
...
func _ready() -> void:
	# The module wires itself from this node's exports on first access.
	# The opening. Also a chart event at t=0; calling it twice is harmless.
	opening()
	# Connect beat.
...
## ─── Per-frame ──────────────────────────────────────────────────────────────
## Nothing here: HUD decay, the camera nudge and the shake all run in
## module._process, which is an autoload and always processing.


## ─── Beat choreography ──────────────────────────────────────────────────────

func _on_beat() -> void:
	if module.dying or clock == null:
		return
	...
	if seconds <= 0.0:
		return
	module.pulse_strumlines(PULSE_TO_PLAYER, PULSE_TO_OPPONENT,
		PULSE_FROM, seconds)
...
func intro_show_text() -> void:
	if not module.first_time(&"intro_show_text") or intro_text == null:
		return
...
func intro_hide_text() -> void:
	if not module.first_time(&"intro_hide_text"):
		return
	if intro_text != null:
...
func intro_reveal() -> void:
	if not module.first_time(&"intro_reveal"):
		return
	if cover != null:
...
func hud_in() -> void:
	if not module.first_time(&"hud_in"):
		return
	module.hud_zoom_in(HUD_IN_ZOOM, HUD_IN_SECONDS)
	module.fade_in_nodes([hud_root, player_lanes], HUD_IN_SECONDS)


func hud_out() -> void:
	if not module.first_time(&"hud_out"):
		return
	var distance: float = HUD_TWEEN_DISTANCE * AnimaniaModule.FUNKIN_TO_RUBICON
	module.tween_hud_out(hud_up, -distance,
		HUD_TWEEN_DELAY_UP, HUD_TWEEN_SECONDS)
	module.tween_hud_out(hud_down, distance,
		HUD_TWEEN_DELAY_DOWN, HUD_TWEEN_SECONDS)


func stand_up() -> void:
	if not module.first_time(&"stand_up"):
		return
	module.swap_characters(stand_cast, stage)
	if hud_root != null:
		hud_root.visible = true


func opponent_lanes_in() -> void:
	if not module.first_time(&"opponent_lanes_in"):
		return
	if opponent_lanes == null or not module._lane_homes.has(&"player"):
		return
	module.lanes_fly_in(opponent_lanes,
		module._lane_homes[&"player"].x,
		LANES_IN_SECONDS, LANES_IN_DELAY, LANES_HALF_ALPHA)


func boyfriend_slide() -> void:
	if not module.first_time(&"boyfriend_slide"):
		return
	module.character_slide(&"boyfriend", SLIDE_DISTANCE, SLIDE_SECONDS)


func keys_on() -> void:
	module.set_keys_enabled(true)
	if script_bars != null:
