...
const ATLAS := "res://holyquintet_mod/source/images/virtualpad/virtualpad.png"
...
const ATLAS_XML := "res://holyquintet_mod/source/images/virtualpad/virtualpad.xml"
...
func _ready() -> void:
	...
	_follow_parent()
	# Bajo un CanvasLayer ajeno (p.ej. la capa 100 que cuelga el gameover/
	# goduka/results encima de la partida), un mando anclado a su capa 80 se
	# quedaria DETRAS de esa capa, tapado por su fondo. Súbese por encima
	# de la capa del padre para que el mando siga siendo lo mas cercano al dedo.
	var pad_parent: Node = get_parent()
	while pad_parent != null and not (pad_parent is CanvasLayer):
		pad_parent = pad_parent.get_parent()
	if pad_parent != null:
		layer = int(pad_parent.layer) + 1
	_atlas = load(ATLAS
	_read_atlas()
	_root = Control.new()
	_root.name = "PadRoot"
...
func _input(event: InputEvent) -> void:
	...
	if touch != null:
		if touch.pressed:
			var hit := _hit(touch.position)
			_press(touch.index, hit)
			if hit != &"":
				get_viewport().set_input_as_handled()
		else:
			var was_held := _held.has(touch.index)
			_press(touch.index, &"")
			if was_held:
				get_viewport().set_input_as_handled()
		return
	...
	if drag != null:
		...
		if _held.has(drag.index):
			var drag_hit := _hit(drag.position)
			_press(drag.index, drag_hit)
			if drag_hit != &"":
				get_viewport().set_input_as_handled()
		return
	...
	if click != null and click.button_index == MOUSE_BUTTON_LEFT:
		...
		if click.device == -1:
			return
		if click.pressed:
			var click_hit := _hit(click.position)
			_press(-1, click_hit)
			if click_hit != &"":
				get_viewport().set_input_as_handled()
		else:
			var click_was := _held.has(-1)
			_press(-1, &"")
			if click_was:
				get_viewport().set_input_as_handled()
