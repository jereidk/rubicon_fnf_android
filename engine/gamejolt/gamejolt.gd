extends Node
## GameJolt API singleton del engine. Portado del addon murikistudio/game-jolt-api.
##
## Configuracion: cada mod declara su bloque "gamejolt" en mod.json y el
## ModLoader llama a GameJolt.configure(dict) al instalar sus autoloads.
## El singleton queda con los valores del mod activo. Sin bloque en el
## manifest, las credenciales quedan vacias y las requests fallan limpias.
##
## Path del autoload: /root/GameJolt. Los mods acceden via
## get_node_or_null("/root/GameJolt") como con cualquier autoload del engine.
##
## API: ver methods publicos abajo. Cubre users/fetch, users/auth, sessions,
## scores (fetch/tables/add/get-rank), trophies, data-store, friends, time
## y batch. Todas las funciones devuelven self para encadenar, y emiten su
## signal *_completed(response) cuando llega la respuesta HTTP.
...
# Built-in overrides
func _ready() -> void:
	# Valores default: sin credenciales. Cuando un mod arranca, el
	# ModLoader llama a configure() con su bloque "gamejolt" del mod.json.
	_debug = false


## Configura el singleton con los valores del mod activo. Llamado por
## ModLoader._install_mod_autoloads() despues de montar el .pck, antes de
## que el mod cargue su escena. Idempotente: llamarlo dos veces
## (por ejemplo, al re-elegir el mismo mod) sobreescribe con los mismos
## valores.
##
## cfg espera:
##   {
...
##     "achievement_map": "res://...",    // opcional, path al GDScript que
##                                        // expone gjid_for(id) -> int
##   }
##
## Si game_id o private_key estan vacios, las requests al API van a
## fallar con "Invalid signature" o similar — pero el resto del mod sigue
## funcionando (el boton gj_button simplemente no va a completar el login).
func configure(cfg: Dictionary) -> void:
	game_id = String(cfg.get("game_id", ""))
	private_key = String(cfg.get("private_key", ""))
	_debug = bool(cfg.get("debug", false))
	if game_id.is_empty() or private_key.is_empty():
		push_warning("[GameJolt] configure(): game_id o private_key vacios — el API no va a funcionar. Revisar mod.json del mod activo.")
	else:
		DebugLog.log("[GameJolt] configurado game_id=%s pk_len=%d" % [game_id, private_key.length()])
...
func users_fetch(username := "", user_ids := []) -> Node:
	var data := {
		"game_id": game_id,
	}
...
func sessions_ping(status := "") -> Node:
	var data := {
		"game_id": game_id,
		"username": user_name,
		"user_token": user_token,
...
func scores_fetch(limit = null, table_id = null, guest := "", better_than = null, worse_than = null, this_user := false) -> Node:
	var data := {
		"game_id": game_id,
	}
...
func scores_add(score: String, sort, table_id = null, guest := "", extra_data = null) -> Node:
	var data := {
		"game_id": game_id,
		"score": score,
		"sort": sort,
...
func scores_get_rank(sort, table_id := "") -> Node:
	var data := {
		"game_id": game_id,
		"sort": sort,
	}
...
func trophies_fetch(achieved = null, trophy_ids := []) -> Node:
	var data := {
		"game_id": game_id,
		"username": user_name,
		"user_token": user_token,
...
func trophies_add_achieved(trophy_id) -> Node:
	var data := {
		"game_id": game_id,
		"username": user_name,
		"user_token": user_token,
...
func trophies_remove_achieved(trophy_id) -> Node:
	var data := {
		"game_id": game_id,
		"username": user_name,
		"user_token": user_token,
...
func data_store_set(key: String, data, global_data := false) -> Node:
...
func data_store_update(key: String, operation: String, value, global_data := false) -> Node:
	var data = {
		"game_id" : game_id,
		"key" : key,
		"operation" : operation,
...
func data_store_remove(key: String, global_data := false) -> Node:
	var data = {
		"game_id" : game_id,
		"key" : key,
	}
...
func data_store_fetch(key: String, global_data := false) -> Node:
	var data = {
		"game_id" : game_id,
		"key" : key,
	}
...
func data_store_get_keys(pattern := "", global_data := false) -> Node:
	var data = {
		"game_id" : game_id,
	}
...
func batch(parallel := false, break_on_error := false) -> Node:
...
func _submit(operation: String, data: Dictionary, optional_data := {}) -> Node:
...
func _dispatch_local_error_response(operation: String, message: String, extra_data := {}) -> Node:
